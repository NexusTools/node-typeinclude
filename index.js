...
function TypeInclude(moduledir) {
    if(!(fs.existsSync(path.resolve(moduledir, "package.json"))))
        throw new Error("new TypeInclude(moduledir) requires a valid nodejs module.");
    var globalClassPath = [processDirectory, path.resolve(moduledir, "node_modules")];
    ...
    this.resolve = typeresolve;
    this.plugin = typeplugin;
    this.clean = typeclean;

    // Global Class Path
    this.classpath = typeclasspath;
    this.addclasspath = typeaddpath;
    this.hasclasspath = typehaspath;
    this.removeclasspath = typeremovepath;
};

module.exports = function(topDir) {
    ...
    instanceFunc.preprocess = instance.preprocess;
    instanceFunc.autocompile = instance.autocompile;
    instanceFunc.registermacro = instance.registermacro;
    instanceFunc.compile = instance.compile;
    instanceFunc.resolve = instance.resolve;
    instanceFunc.plugin = instance.plugin;
    instanceFunc.clean = instance.clean;
    instanceFunc.path = instance.path;

    // Global Class Path
    instanceFunc.classpath = instance.classpath;
    instanceFunc.addclasspath = instance.addclasspath;
    instanceFunc.hasclasspath = instance.hasclasspath;
    instanceFunc.removeclasspath = instance.removeclasspath;
    
    global.__typeinclude__.modulecache[topDir] = instanceFunc;
    return instanceFunc;
}
